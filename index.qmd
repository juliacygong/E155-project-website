...
The system block diagram shows the flow of our design, starting with an input from a microphone which is sampled by the MCU's internal ADC. The MCU's ADC output is sent over to the FPGA using SPI. The music score and notes are output the FPGA to the VGA monitor. 
